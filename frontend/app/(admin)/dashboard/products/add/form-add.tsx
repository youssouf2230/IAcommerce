...
const ProductForm = ({ categories }: { categories: Category[] }) => {
  ...
  return (
    <form id="product-form" action={handleSubmit} className="space-y-4 p-6">
      ...
      <InputForm label="Sell Price" id="sellPrice" name="sellPrice" min={0} defaultValue={0} type="number" step="0.01" error={state?.errors?.properties?.sellPrice?.errors[0]} />
      <InputForm label="Old Price" id="oldPrice" name="oldPrice" min={0} defaultValue={0} type="number" step="0.01" error={state?.errors?.properties?.oldPrice?.errors[0]} />
      <InputForm label="Stock Quantity" type="number" min={0} id="stockQuantity" name="stockQuantity" error={state?.errors?.properties?.stockQuantity?.errors[0]} />
      <TextareaForm label="Description" id="description" name="description" error={state?.errors?.properties?.description?.errors[0]} />
      <SelectForm label="Category" id="categoryId" name="categoryId" options={categories} error={state?.errors?.properties?.categoryId?.errors[0]} />
      ...
      {state?.errors && <p className="text-sm text-red-500">{state?.errors?.error}</p>}
